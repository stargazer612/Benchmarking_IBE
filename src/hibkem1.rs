...
impl HIBKEM1 {
    ...
    pub fn setup(&self) -> (HIBKEM1PublicKey, HIBKEM1DelegationKey, HIBKEM1SecretKey) {
        ...
        let pk = HIBKEM1PublicKey { a_g1, z_g1, z_prime_g1 };
        let dk = HIBKEM1DelegationKey { b_g2, d_g2, e_g2 };
        let sk = HIBKEM1SecretKey { sk_mac, y_matrices, y_prime };

        (pk, dk, sk)
    }

    pub fn extract(&self, sk: &HIBKEM1SecretKey, id: &[Vec<u8>]) -> (HIBKEM1UserSecretKey, HIBKEM1UserDelegationKey) {
        let p = id.len();
    ...
    pub fn delegate(&self, dk: &HIBKEM1DelegationKey, usk: &HIBKEM1UserSecretKey, udk: &HIBKEM1UserDelegationKey, id_prefix: &[Vec<u8>], id_next: Vec<u8>) -> (HIBKEM1UserSecretKey, HIBKEM1UserDelegationKey) {
        let p = id_prefix.len();
        assert!(p > 0 && p < self.max_levels);
        ...
        let usk_prime = HIBKEM1UserSecretKey {t_g2: t_prime_g2, u_g2: u_prime_g2, v_g2: v_prime_g2};
        let udk_prime = HIBKEM1UserDelegationKey {d_g2: d_prime, e_g2: e_prime};
