...
fn common_runtimes() {
    ...
    let group = GroupCtx::bls12_381();
    let g1_matrix: Vec<Vec<G1Projective>> = (0..20)
        .map(|_| {
            (0..20)
    ...
    let g2_matrix: Vec<Vec<G2Projective>> = (0..20)
        .map(|_| {
            (0..20)
    ...
    println!(
        "generate_email_and_hash_identity (128 bits): {:?}\n",
        start.elapsed()
    );
}
